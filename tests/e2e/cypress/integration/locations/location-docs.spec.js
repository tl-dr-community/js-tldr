        ...
        cy.fixture('docspaths').then((object) => {
            object.domains.forEach(function(domain) {
                ...
                cy.location().should((location) => {
                    ...
                    expect(location.href).to.eq(`http://localhost:3000${path}/${domain.methods.prototype[0]}`)
                    expect(location.host).to.eq('localhost:3000')
                    expect(location.hostname).to.eq('localhost')
                    expect(location.origin).to.eq('http://localhost:3000')
                    expect(location.pathname).to.eq(`${path}/${domain.methods.prototype[0]}`)
    ...
    it('cy.url() - get the current URL', () => {
        cy.fixture('docspaths').then((object) => {
            object.domains.forEach(function(domain) {
                ...
                cy.url().should('eq', `http://localhost:3000${path}/${domain.methods.prototype[0]}`)
            })
        })
    })
})